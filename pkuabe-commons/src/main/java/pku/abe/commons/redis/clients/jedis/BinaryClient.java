...
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.LEN;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.LIMIT;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.NO;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.ONE;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.REFCOUNT;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.RESET;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.STORE;
import static pku.abe.commons.redis.clients.jedis.Protocol.Keyword.WITHSCORES;
...
import pku.abe.commons.redis.clients.jedis.Protocol.Command;
import pku.abe.commons.redis.clients.jedis.Protocol.Keyword;
...
public class BinaryClient extends Connection {
    ...
    protected void lsset(final byte[] key, final int length, final byte[] values) {
        sendCommand(LSSET, key, toByteArray(length), values);
    }

    protected void lsput(final byte[] key, final long[] values) {
        byte[][] params = new byte[values.length + 1][];
        params[0] = key;
        int i = 1;
        for (long v : values) {
            params[i] = toByteArray(v);
            i++;
        }
        sendCommand(LSPUT, params);
    }

    protected void lsdel(final byte[] key, final long[] values) {
        byte[][] params = new byte[values.length + 1][];
        params[0] = key;
        int i = 1;
        for (long v : values) {
        ...
        sendCommand(LSDEL, params);
    }

    protected void lsmexists(final byte[] key, final long[] values) {
        byte[][] params = new byte[values.length + 1][];
        params[0] = key;
        int i = 1;
        for (long v : values) {
        ...
        sendCommand(LSMEXISTS, params);
    }

    protected void lsgetall(final byte[] key) {
        sendCommand(LSGETALL, key);
    }

    protected void lslen(final byte[] key) {
        sendCommand(LSLEN, key);
    }
    ...
    public void bfget(byte[] key) {
        sendCommand(BFGET, key);
    }

    public void bfset(byte[] key) {
        sendCommand(BFSET, key);
    }

    public void bfmget(byte[]... keys) {
        sendCommand(BFMGET, keys);
    }

    public void bfmset(byte[]... keys) {
        sendCommand(BFMSET, keys);
    }
}
