...
public class Pipeline extends Queable {
    ...
    /**
     * Syncronize pipeline by reading all responses. This operation close the pipeline. In order to
    ...
    /**
     * Syncronize pipeline by reading all responses. This operation close the pipeline. Whenever
     * possible try to avoid using this version and use Pipeline.sync() as it won't go through all
     * the responses and generate the right response type (usually it is a waste of time).
     *
    ...
    public Response<byte[]> hgetByteArray(byte[] key, byte[] field) {
        client.hget(key, field);
        return getResponse(BuilderFactory.BYTE_ARRAY);
    }
    ...
    public Response<Map<byte[], byte[]>> hgetAllBytes(byte[] key) {
        client.hgetAll(key);
    ...
    public Response<List<byte[]>> hmgetBytes(byte[] key, byte[]... fields) {
        client.hmget(key, fields);
    ...
    public Response<Set<byte[]>> zrevrangeByScoreX(byte[] key, byte[] max, byte[] min, int offset, int count) {
        client.zrevrangeByScore(key, max, min, offset, count);
    ...
    public Response<Long> bfset(String key) {
        client.bfset(key);
        return getResponse(BuilderFactory.LONG);
    }

    public Response<Long> bfget(String key) {
        client.bfget(key);
        return getResponse(BuilderFactory.LONG);
    }

    public Response<List<Long>> bfmget(String... keys) {
        client.bfmget(keys);
        return getResponse(BuilderFactory.LONG_LIST);
    }

    public Response<List<Long>> bfmset(String... keys) {
        client.bfmset(keys);
        return getResponse(BuilderFactory.LONG_LIST);
    }
