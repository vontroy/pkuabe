...
public class Client extends BinaryClient implements Commands {
    ...
    public void lsset(String key, int length, byte[] bytes) {
        lsset(SafeEncoder.encode(key), length, bytes);
    }

    public void lsput(String key, long[] value) {
        lsput(SafeEncoder.encode(key), value);
    }

    public void lsdel(String key, long[] value) {
        lsdel(SafeEncoder.encode(key), value);
    }

    public void lsmexists(String key, long[] values) {
        lsmexists(SafeEncoder.encode(key), values);
    }

    public void lsgetall(String key) {
        lsgetall(SafeEncoder.encode(key));
    }

    public void lslen(String key) {
        lslen(SafeEncoder.encode(key));
    }
    ...
    public void pfadd(String key, String... elements) {
        pfadd(SafeEncoder.encode(key), strs2BytesArr(elements));
    }

    public void pfcount(String... keys) {
        pfcount(strs2BytesArr(keys));

    }

    public void pfmerge(String destkey, String... sourcekeys) {
        pfmerge(SafeEncoder.encode(destkey), strs2BytesArr(sourcekeys));
    }

    private byte[][] strs2BytesArr(String... params) {
        final byte[][] bparams = new byte[params.length][];
        for (int i = 0; i < bparams.length; i++) {
            bparams[i] = SafeEncoder.encode(params[i]);
        }
        return bparams;
    }
    ...
    public void bfget(final String key) {
        bfget(SafeEncoder.encode(key));
    }

    public void bfset(final String key) {
        bfset(SafeEncoder.encode(key));
    }
