...
import pku.abe.commons.profile.ProfileLoggerUtil;
...
public class StatLog implements Runnable {
    ...
    private static Map<String, Ehcache> ehcaches = new ConcurrentHashMap<String, Ehcache>();

    private static Set<String> cacheStatKeys = new HashSet<String>();

    static {
        printStat(5000);
        printExecutorStat(10000);
    }
    ...
    public static void registerExecutor(String name, ThreadPoolExecutor executor) {
        if (StringUtils.isNotEmpty(name)) {
            executor.setThreadFactory(new ThreadFactoryBuilder().setNameFormat(name).build());
        }
        executors.put(name, executor);
    }

    public static void registerEhCache(String name, Ehcache ehcache) {
        ehcaches.put(name, ehcache);
    }
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                if (pausePrint.get()) continue;

                long time2 = System.currentTimeMillis();
                if (time2 == 0) break;
                if (time2 == startTime) continue;
                cnt = count.get();
                long cur = (cnt - lastCount) * 1000l / (time2 - lastTime);
                if (cur > max) max = cur;

                log.info("---------------------------");
                log.info("JAVA HEAP: " + memoryReport() + ", UP TIME: " + ((time2 - startTime) / 1000) + ", min: "
                        + ((time2 - startTime) / 60000));
                ...
                // stat ehcache
                for (Map.Entry<String, Ehcache> entry : ehcaches.entrySet()) {
                    log.info(statEhCache(entry.getKey(), entry.getValue()));
                }
    ...
    private static String statProfileExecutor(String name, ThreadPoolExecutor executor) {
        JsonBuilder jb = new JsonBuilder();
        jb.append("type", ProfileType.JVM.name());
        jb.append("name", String.format("EXECUTOR_%s", name.toUpperCase()));
        jb.append("QueueSize", executor.getQueue().size());
        jb.append("CompletedTaskCount", executor.getCompletedTaskCount());
        jb.append("TaskCount", executor.getTaskCount());
        jb.append("ActiveCount", executor.getActiveCount());
        jb.append("CorePoolSize", executor.getCorePoolSize());
        return jb.flip().toString();
    }

    /**
     * print internal status of all thread pools
     * 
     * @param interval
     */
    private static void printExecutorStat(int interval) {
        Executors.newScheduledThreadPool(1).scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                for (Map.Entry<String, ThreadPoolExecutor> entry : executors.entrySet()) {
                    ProfileLoggerUtil.monitor(statProfileExecutor(entry.getKey(), entry.getValue()));
                }
            }
