...
public interface CacheFactoryStrategy {
    ...
    /**
     * Returns true if the cluster has been started. When running in local mode a true value should
     * be returned.
     * <p>
     * <p>
    ...
    /**
     * Creates a new cache for the cache name specified. The created cache is already configured.
     * Different implementations could store the cache configuration in different ways. It is
     * recommended to store the cache configuration in an external file so it is easier for
     * customers to change the default configuration.
     *
    ...
    /**
     * Returns true if this node is the maste node of the cluster. When not running in cluster mode
    ...
    /**
     * Returns the maximum number of cluster members allowed. A value of 0 will be returned when
    ...
    /**
     * Updates the statistics of the specified caches and publishes them into a cache for
     * statistics. The statistics cache is already known to the application but this could change in
     * the future (?). When not in cluster mode then do nothing.
     * <p>
     * <p>
     * The statistics cache must contain a long array of 5 positions for each cache with the
     * following content:
     * <ol>
     ...
     * operation is thread safe. The supplied cache may or may not be used depending whether the
     * server is running on cluster mode or not. When not running as part of a cluster then the lock
     * will be unrelated to the cache and will only be visible in this JVM.
     *
